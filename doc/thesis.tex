...
\begin{figure}[h!]
    \centering
    \includegraphics[width=\linewidth]{lime-view.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=\linewidth]{whole_interface.png}
...
\begin{figure}[h!] % TODO: h! musi być wyłączone
    \centering
    \includegraphics[width=8cm]{sidebar.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=12cm]{spectrogram-attribution.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=12cm]{spectrogram-raw.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=12cm]{attribution_map_frontend.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=8cm]{version-information.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=8cm]{notification-panel.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=12cm]{model-information.png}
...
\begin{figure}%[h!] % TODO: h! musi być wyłączone
    \centering
    \includegraphics[width=15cm]{ClassDiagram.png}
...
\subsubsection{Integrated gradients method adapter}
    ...
    Additionally, the \mintinline{py}{igrad_prepare_inference_input()} method is defined by the Integrated gradients explainer,
...
This method must return a function with a type \mintinline{py}{Callable[[torch.Tensor], torch.Tensor]},
...
a function object that must contain the instance of Python's \mintinline{py}{torch.nn.Module} that
...
a Python's type \mintinline{py}{torch.nn.Module} and provides a required
...
The \mintinline{py}{explain()} method is the top-level function used to launch the explanation process.
...
It also launches the specified view class using \mintinline{py}{self.view.start()},
...
The internal \mintinline{py}{explain_instance()} method executes the user’s provided input and passes
...
The method \mintinline{py}{explain_instance_visualize()} is similar to the \emph{explain\_instance}, 
...
To prevent the interactive window of the generated image, a call to the function \mintinline{py}{plt.ioff()}
...
The method \mintinline{py}{get_smoothed_attribution()} of the explainer is the helper
...
it is \mintinline{py}{window_size=15}, using a sliding window, as shown in Fig. \ref{fig:get_smoothed_attribution}.
...
The \texttt{LimeExplainer} contains only a top-level \mintinline{py}{explain()} method,
...
the \mintinline{py}{adapter.get_lime_predict_fn()} adapter handler as shown in Fig. \ref{fig:LimeExplExecution}.
...
the \mintinline{py}{adapter.get_lime_predict_fn()} adapter handler, as shown in Fig. \ref{fig:LimeGetSortedComps}.
...
\begin{itemize}
    ...
    \item \mintinline{py}{write_plt_image(self, fig, suffix)} - it handles saving the results of Matplotlib's (Sec. \ref{ch:ToolsSpecification}) generated image,
...
\acronym{MVP}{Model-View-Presenter}
...
% Załącznik 1
\clearpage
\appendix{The installation script \emph{setup.sh}} \label{appendix:InstallationScriptListing}
...
\end{minted}
\begin{minted}[numbersep=12pt, fontsize=\scriptsize, firstnumber=59, breaklines, xleftmargin=12pt, linenos, mathescape]{py}
pip install tqdm
